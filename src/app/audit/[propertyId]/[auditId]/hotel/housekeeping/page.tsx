  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">Section 3 of 10</p>
      </div>
      <HotelSectionForm auditId={auditId} sectionKey="housekeeping" />
      <div className="flex items-center justify-between pt-2">
        <Button variant="outline" onClick={() => router.back()}>← Back</Button>
        <Button onClick={() => router.push(`/audit/${propertyId}/${auditId}/hotel/engineering`)}>
          Next: Engineering <ArrowRight className="h-4 w-4" />
        </Button>
      </div>
    </div>
  );
