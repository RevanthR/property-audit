  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">Section 9 of 10</p>
      </div>
      <HotelSectionForm auditId={auditId} sectionKey="humanResources" />
      <StepFooter>
        <Button variant="outline" onClick={() => router.back()}>← Back</Button>
        <Button onClick={() => router.push(`/audit/${propertyId}/${auditId}/hotel/guest-experience`)}>
          Next: Guest Experience <ArrowRight className="h-4 w-4" />
        </Button>
      </StepFooter>
    </div>
  );
